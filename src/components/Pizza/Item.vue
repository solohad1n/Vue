  ...
  <div v-if="pizza" class="pizza-block">
    ...
    <div class="pizza-block__bottom">
      ...
      >
        ...
        <span>Добавить</span>
        <i v-if="addedPizzaCount">{{addedPizzaCount}}</i>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    const addedPizzaCount = computed(
      () => store.state.cartItems.get(props.pizza.id)?.count
    );

    const handleAddPizzaToCart = () => {
      store.commit("ADD_PIZZA_TO_CART", props.pizza);
    };
    return { handleAddPizzaToCart, addedPizzaCount };
  },
};
</script>
